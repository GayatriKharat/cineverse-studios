...
export default async function CraftPage({ params }: { params: Promise<{ slug: string; craft: string }> }) {
  ...
  return (
    <main>
      ...
      <section className="craft-actions wrap">
        <Link className="button" href={`/contact?service=${encodeURIComponent(slug)}&craft=${encodeURIComponent(craftSlug)}`}>
          Start with this craft ↗
        </Link>
