  ...
  return (
    <main>
      ...
      <section className="resources-architecture wrap">
        ...
        <section id="articles" className="resource-section"><div className="resource-feature-head"><div><h2>Longer form, <em>deeper craft.</em></h2></div><Link className="text-link" href="/resources/articles">All articles ↗</Link></div><div className="editorial-list resource-preview-list">{articles.map((article) => <Link key={article.slug} href={`/resources/articles/${article.slug}`}><span>{article.tag}</span><div><h3>{article.title}</h3><small>{article.meta}</small></div><b>↗</b></Link>)}</div></section>
        <section id="voices" className="resource-section resource-voices"><div className="resource-feature-head"><div><h2>What clients <em>say.</em></h2></div><Link className="text-link" href="/resources/testimonials">All testimonials ↗</Link></div><div className="resource-testimonial-grid">{testimonials.map((item) => <article key={item.name}><span className="stars" aria-label="5 stars">★★★★★</span><blockquote>“{item.quote}”</blockquote><footer><b>{item.name}</b><small>{item.scope}</small></footer></article>)}</div></section>
        <section id="answers" className="resource-section resource-answers"><div className="resource-feature-head"><div><h2>Questions we get <em>often.</em></h2></div><Link className="text-link" href="/resources/faqs">All FAQs ↗</Link></div><div className="resource-faq-list">{faqs.map(([question, answer]) => <details key={question}><summary>{question}<span>+</span></summary><p>{answer}</p></details>)}</div></section>
      </section>
      <CtaBand />
    </main>
  );
