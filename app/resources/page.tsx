  ...
  return (
    <main>
      ...
      <section className="resources-architecture wrap">
        <div className="resource-tabs" aria-label="Resource categories"><Link href="/resources/articles">Articles</Link><Link href="/resources/news">News</Link><Link href="/resources/faqs">FAQs</Link><Link href="/resources/testimonials">Testimonials</Link></div>
        <div className="resource-feature-head"><div><p className="eyebrow">01 / Journal</p><h2>Notes from <em>the floor.</em></h2></div></div>
        ...
        <div className="resource-columns">
          <section><p className="eyebrow">03 / Voices</p><h2>What clients <em>say.</em></h2>{testimonials.slice(0, 2).map((item) => <blockquote key={item.name}>“{item.quote}”<small>{item.name} · {item.scope}</small></blockquote>)}<Link className="text-link" href="/resources/testimonials">All testimonials ↗</Link></section>
