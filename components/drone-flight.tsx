...
function CinemaDrone(){
  ...
  return <Float speed={.65} rotationIntensity={.03} floatIntensity={.12}><group ref={craft} scale={1.05}>
    ...
    <pointLight position={[0,-.32,.8]} color="#8B5CF6" intensity={5} distance={3}/><pointLight position={[0,.18,-.8]} color="#d9ecef" intensity={2} distance={2}/>
  </group></Float>;
}
export function DroneFlight(){return <div className="drone-flight" aria-hidden="true"><Canvas camera={{position:[0,1.3,7],fov:43}} dpr={[1,1.5]} gl={{alpha:true,antialias:true}}><ambientLight intensity={.5}/><spotLight position={[2,5,4]} intensity={80} color="#8B5CF6" angle={.45}/><CinemaDrone/><Environment preset="city"/></Canvas></div>}
